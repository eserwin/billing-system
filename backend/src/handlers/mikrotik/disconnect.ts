...
import { customerIdParamSchema, disconnectBodySchema } from '../../validators/mikrotikSchema';
import { disconnect } from '../../services/mikrotikService';
...
async function handler(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  ...
  const result = await disconnect(id, reason, auth?.userId);
...
export const main = middy(handler)
  ...
  .use(validator({ pathParameters: customerIdParamSchema, body: disconnectBodySchema }))
  .use(errorHandler());
